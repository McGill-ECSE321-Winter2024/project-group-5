...
export default {
  ...
  methods: {
    ...
    validateEmail() {
      ...
      const hasInvalidChars = !email
        .split("")
        .every((char) => allowedCharacters.includes(char.toLowerCase()));
      const isClientWithSportPlusEmail =
        this.userType === "Client" && email.endsWith("@sportplus.com");
      const isInstructorNotWithSportPlusEmail =
        this.userType === "Instructor" && !email.endsWith("@sportplus.com");
      ...
      if (
        ...
        !endsWithValidDomain ||
        hasInvalidChars ||
        isClientWithSportPlusEmail ||
        isInstructorNotWithSportPlusEmail
      ) {
        ...
        if (!email) this.emailFeedback = "Email cannot be empty.";
          ...
          this.emailFeedback = 'Email can only have 1 "@" symbol.';
        else if (!endsWithValidDomain)
          this.emailFeedback = "Email can only end with .com or .ca.";
        else if (startsWithAt)
          this.emailFeedback = 'Email cannot start with "@".';
        else if (hasDotAt)
          this.emailFeedback = 'Email cannot contain ".@" or "@.".';
        else if (hasInvalidChars)
          this.emailFeedback = "Email contains invalid characters.";
        else if (isClientWithSportPlusEmail)
          this.emailFeedback =
            'Clients cannot have an email ending with "@sportplus.com".';
        else if (isInstructorNotWithSportPlusEmail)
          this.emailFeedback =
            'Instrcutors can only have an email ending with "@sportplus.com".';
    ...
    async checkEmailExists(email) {
      try {
        ...
        if (this.userType === "Client") {
          ...
          if (
            response.data &&
            (!response.data.firstName || response.data.firstName.trim() === "")
          ) {
